...
import com.example.fawrydemo.utils.ServiceFactory.VodafoneCash;
...
public class VodafoneCashService {
    VodafoneCash vodafoneCash= new VodafoneCash();
...
//        if logged in
        ArrayList<String> arr =vodafoneCash.getForm().getReq();
    ...
    public ResponseEntity<?> pay(VodafoneCashForm form){
        ObjectNode response = mapper.createObjectNode();
...
//        if no user logged in
        ...
        if(form.getAmount()==0 || form.getPhoneNumber()==null){
            response.put("message","requirements missing or amount 0");
            return ResponseEntity.status(400).body(response);
        }

        IHandler handler=vodafoneCash.getHandler();
        handler.setForm(form);
        handler.setProvider(vodafoneCash.getProvider());
    ...
    public ResponseEntity<?> walletPay(VodafoneCashForm form){
        ObjectNode response = mapper.createObjectNode();
//        if admin is logged in
        if(GlobalUser.getGlobalUser().getUserMode() == 0){
            response.put("message","admins not allowed");
...
//        if no user logged in
    ...
    public ResponseEntity<?> addDiscount(double discount) {
        ...
        vodafoneCash.getHandler().setSpecificDiscount(discount);
        response.put("message","success");
        return ResponseEntity.ok(response);
    }
    public ResponseEntity<?> removeDiscount() {
        ...
        vodafoneCash.getHandler().setSpecificDiscount(0);
        response.put("message","success");
        return ResponseEntity.ok(response);
    }
}